...
class ShoppingCartSpec extends AnyFunSpec with Matchers with Fixture{
    ...
    describe("Add products to an empty Shopping Cart") {
        describe("Add one product") {
            it("should contain one Product") {
                val result = shoppingCartService.addProduct(emptyShoppingCart, doveProduct, Quantity(1))
        ...
        describe("Add one product with quantity equals 0") {
            it("should be empty") {
                val result = shoppingCartService.addProduct(emptyShoppingCart, doveProduct, Quantity(0))
        ...
        describe("Add two different products") {
            it("should contain two products") {
                val result = shoppingCartService
                  .addProduct(
                    shoppingCartService.addProduct(emptyShoppingCart, doveProduct, Quantity(1)),
    ...
    describe("Step 1 - Add five products to an empty Shopping Cart") {
        it("should contain five Products") {
            val result = shoppingCartService.addProduct(emptyShoppingCart, doveProduct, Quantity(5))
            result.items.size shouldEqual 1
    ...
    describe("Step 2 - Add additional products to an empty Shopping Cart (5 plus 3)") {
        it("should contain eight Products") {
            val partialResult = shoppingCartService.addProduct(emptyShoppingCart, doveProduct, Quantity(5))
            val result = shoppingCartService.addProduct(partialResult, doveProduct, Quantity(3))
            result.items.size shouldEqual 1
            val quantity = result.items.get(doveProduct)
    ...
    describe("Step 3 - Calculate the tax rate") {
        ...
        describe("calculate the tax rate for a shopping cart with multiple products") {
            it("should contain 4 Products and valid tax") {
                val partialResult = shoppingCartService.addProduct(emptyShoppingCart, doveProduct, Quantity(2))
                val result = shoppingCartService.addProduct(partialResult, axeProduct, Quantity(2))
...
object ShoppingCartSpec {
    trait Fixture {
        ...
        val shoppingCart = ShoppingCart(shoppingCartId = 1, items = items, total = 39.99, totalWithTaxes = 44.99, tax = 5.0)

        val emptyShoppingCart = shoppingCart.copy(items = Map.empty, total = 0.0, tax = 0.0)

        val shoppingCartWithTwoProducts =
            shoppingCart.copy(items = Map(doveProduct -> Quantity(1), doveProduct.copy(productId = 2, name = "Ivory") -> Quantity(1)), total = 79.98, totalWithTaxes = 89.98, tax = 10.0)

        val shoppingCartWithFiveDoveSoaps = shoppingCart.copy(items = Map(doveProduct -> Quantity(5)), total = 199.95, totalWithTaxes = 224.94,tax = 24.99)

        val shoppingCartWithEightDoveSoaps = shoppingCart.copy(items = Map(doveProduct -> Quantity(8)), total = 319.92, totalWithTaxes = 359.91, tax = 39.99)

        val shoppingCartWithTwoDoveAndTwoAxe = shoppingCart.copy(items = Map(doveProduct -> Quantity(2), axeProduct -> Quantity(2)), total = 279.96, totalWithTaxes = 314.96, tax = 35.0)
